...
export class TabComparativeComponent implements OnInit {
  ...
  desde: number = 0;
  hasta: number = 8
  constructor(private comparativeService: ComparativeService,
  ...
  ngOnInit(): void {
    ...
    this.comparativeService.getAllTipoComercio().subscribe(responseTipoComercio => {
      this.tipoComercio = responseTipoComercio;
    });

    this.comparativeService.getAllTipoServicio().subscribe(responseTipoServicio => {
      this.tipoServicio = responseTipoServicio;
    });
    this.comparativeService.getAllTerminalPortuario().subscribe(responseTerminalPortuario => {
      this.terminalPortuario = responseTerminalPortuario;
    });

    this.comparativeService.getAllCircunscripcionAduanera().subscribe(responseCircunscripcionAduanera => {
    ...
    this.comparativeService.getAllTipoViaProcedencia().subscribe(responseTipoViaProcedencia => {
      this.tipoViaProcedencia = responseTipoViaProcedencia;
    });
    this.comparativeService.getAllModalidadTransporte().subscribe(responseModalidadTransporte => {
    ...
    this.comparativeService.getAllDocTransporte().subscribe(responseDocTransporte => {
    ...
    this.comparativeService.getAllMoneda().subscribe(responseMoneda => {
      ...
      this.dolarSoles = this.moneda.filter(item => item.nombre === 'SOLES' || item.nombre === 'DÓLARES');
  ...
  private setUniqueOperator(arrayData: any[], operator: any) {
  ...
  imprimir() {
    ...
    let contenido = '<h2>' + this.getRazonSocial + '</h2><table><thead><tr><th>Servicio</th><th>Descripción</th><th>Moneda</th><th>Precio</th></tr></thead><tbody>';

    for (let i = 0; i < this.serviciosEmpresa.length; i++) {
      contenido += '<tr><td>' + this.serviciosEmpresa[i].denominacionServicio + '</td><td>' + this.serviciosEmpresa[i].descripcion + '</td><td>' + this.serviciosEmpresa[i].moneda + '</td><td>' + this.serviciosEmpresa[i].precio + '</td></tr>';
    }
    contenido += '</tbody></table>';
    let estilo = "<style>table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid black; padding: 5px; } th:nth-child(3), td:nth-child(3) { width: 100px; } th:nth-child(4), td:nth-child(4) { width: 80px; }</style>";
  ...
  getFiltrosAplicados(code: any, value: any, filterKey: string) {
    let resultado = this.selectToChips.find((selectToChips) => selectToChips.tipo == filterKey) ?? null;
  ...
  buscar() {
    ...
    if (this.selectToChips != null && this.selectToChips != undefined) {
      this.comparativeService.getFiltroTipoComparative(this.selectToChips).subscribe(response => {
          this.adminConce = this.getOperator(response.detalles);

          this.ordenarData = this.adminConce.sort((a, b) => a.razonSocial.localeCompare(b.razonSocial))
  ...
  ordenamiento(selectValue: any) {
    ...
    this.comparativeService.getFiltroTipoComparative(this.selectToChips).subscribe(response => {
      ...
      if (selectValue === 'A-z') {
        ...
        this.ordenarData = this.adminConce.sort((a, b) => b.razonSocial.localeCompare(a.razonSocial))
      }
    });
  }
  ...
  verEmpresa(idEmpresa: any, razonSocial: any) {
    ...
    if (monedaId !== null) {
      this.comparativeService.getServices(monedaId, this.selectToChips).subscribe(response => {
  ...
  onSelect(id: any) {
    ...
    if (id == 20) {
      ...
      this.misDatosSelect["opeTipOpeCircunscripcionId"] = this.circunscripcion_aduanera;
  ...
  cambiarPagina(e: PageEvent) {
    ...
    this.hasta = endIndex > this.serviciosEmpresa.length ? this.serviciosEmpresa.length : endIndex;
  ...
  cambiarPaginaEmpresa(e: PageEvent) {
    ...
    this.hastaEmpresa = endIndex > this.adminConce.length ? this.adminConce.length : endIndex;
